...
def getMovDoi(soup):
    try:
        ...
        return soup.find('div',class_="credits_r").text
    except:
        return None
'''
aact=[]
for b in range(len(getMovDoi(soup))):
          aact.append(getMovDoi(soup).find_all("a")[b])
#print(getMovDoi(soup).find_all("a")[1].string)
'''


def getMovText(getMovDoi,key):
    try:    
        return str(re.compile(key +".*").findall(str(getMovDoi))[0])[len(key)+2:]
    except:
        return None

def getMovText1(getMovDoi,key):
    try:    
        return str(re.compile(key +"*.").findall(str(getMovDoi))[0])[len(key)+2:]
    except:
        return None

getMovDoi=getMovDoi(soup)
#print(getMovText(getMovDoi,"导演"))

print(getMovText1(getMovDoi,"编剧"))
